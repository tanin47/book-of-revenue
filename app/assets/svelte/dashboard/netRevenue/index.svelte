...
import FilterDialog, {BASE_COLUMNS, COLUMNS, type Params, SHOW_ONLYS} from './_filter_dialog.svelte';
...
async function exportCsv(): Promise<void> {
  ...
  try {
    const json = await post(`/export-net-revenue`, {
      params: {...params, currency: CURRENT_CURRENCY},
    })
...
function computeLink(
  ...
  switch (params.groupBy) {
    case 'Summary':
      return `/net-revenue?${generateQueryString({...opts, groupBy: 'Customer'})}`
    case 'Product':
      return `/net-revenue?${generateQueryString({...opts, groupBy: 'LineItem', productId: dataRow[dataColumnIndexById.ProductId]})}`
    case 'Customer':
    ...
    <div class="flex flex-col grow w-full items-stretch overflow-hidden relative">
          ...
          <div class="text-xs px-2 py-1 flex items-center gap-2">
            ...
            {#if params.showOnly}
              <div class="flex items-center gap-1">
                ...
                <span>{SHOW_ONLYS.find(s => s.id === params.showOnly)?.label}</span>
              </div>
            {/if}
            {#if params.productId}
              <div class="flex items-center gap-1">
      ...
      <Table
        ...
        columnRenderingSettings={{
          ...
          'GrossRevenue': {computeLink},
          'CreditNotes': {computeLink},
          'Refunds': {computeLink},
          'Voids': {computeLink},
          'Disputes': {computeLink},
          'NetRevenue': {computeLink},
        }}
        onFetch={load}
        onFetchMore={fetch}
      />
