...
export type Page = 'net-revenue' | 'deferred-revenue' | 'other-contractual-liabilities'
</script>
<script lang="ts">
...
const METRIC_LINKS = [
  ...
  {value: 'other-contractual-liabilities', name: 'Other contractual liability change'},
]
</script>
    ...
    <div class="flex gap-3 items-center px-2 py-1">
      ...
      <div class="flex items-center gap-2 font-bold">
        ...
        <select
          ...
          class="select select-xs select-bordered bg-base-100"
          onchange={(ev) => {
            window.location.href = `/customer/${customerId ?? 'empty'}/${(ev.target as any).value}`
          }}
        >
