...
export type Params = TableParams & {
  ...
  export const BASE_COLUMNS: ColumnSelectionItem[] = [
    ...
    {id: 'Category', forceChecked: true, rank: 1},
    {id: 'Account', forceChecked: true, rank: 2},
...
async function fetchAllAccounts(): Promise<void> {
  const json = await post('/load-balance-sheet-accounts', {})
...
$: {
  ...
  if (groupBy2 !== 'Event') {
    possibleColumns = possibleColumns.filter(c => c.id !== 'Event')
    selectedColumns = selectedColumns.filter(c => c !== 'Event')
  }
...
<dialog bind:this={modal} class="modal2">
  <div class="modal-box !min-w-[500px] !w-auto !max-w-none flex flex-col gap-2 text-sm">
    ...
    {#if showOnly || productId || customerId || transactionId}
      <div class="flex gap-2 items-center">
        <span class="font-bold">Advance filters:</span>
        <div class="flex gap-2 items-center">
          {#if showOnly}
            <div class="flex items-center gap-1 rounded bg-accent text-white text-xs px-2 py-1">Non-zero: {showOnly}</div>
          {/if}
          {#if productId}
