...
function generateQueryString(
  ...
  const accounts = overrides?.accounts ?? params.accounts;
  if (accounts.length > 0) {
    q.append('accounts', accounts.join(','));
  }
...
function updateParamsFromQueryString() {
  ...
  try {
    ...
    params.groupBy2 = q.get('with') ?? params.groupBy2
...
async function fetch(params: Params, offset: number): Promise<FetchResult> {
  return post('/load-balance-sheet', {
...
async function exportCsv(): Promise<void> {
  ...
  try {
    const json = await post(`/export-balance-sheet`, {
      params: {...params, currency: CURRENT_CURRENCY},
    })
...
function computeLink(
  ...
  const opts = {period: dataRow[dataColumnIndexById.AccountingPeriod], accounts: [dataRow[dataColumnIndexById.Account]], showOnly: columnId}
  const base = 'balance-sheet'
  switch (params.groupBy) {
    case 'Summary':
      return `/${base}?${generateQueryString({...opts, groupBy: 'Customer'})}`
    case 'Product':
      return `/${base}?${generateQueryString({...opts, groupBy: 'LineItem', productId: dataRow[dataColumnIndexById.ProductId]})}`
    case 'Customer':
      return `/${base}?${generateQueryString({...opts, groupBy: 'Transaction', customerId: dataRow[dataColumnIndexById.CustomerId]})}`
    case 'Transaction':
      return `/${base}?${generateQueryString({...opts, groupBy: 'LineItem', transactionId: dataRow[dataColumnIndexById.RevRecTransactionId]})}`
    case 'LineItem':
      return null
    default:
      return null
    ...
    <div class="flex flex-col grow w-full items-stretch overflow-hidden relative">
      <div class="flex justify-between gap-2 items-stretch bg-base-200 text-sm text-primary border-base-content border-b">
        <div class="flex items-stretch gap-0">
          ...
          <div class="text-xs px-2 py-1 flex items-center gap-2">
            ...
            {#if params.showOnly}
              <div class="flex items-center gap-1">
                <span class="font-bold">Non-zero:</span>
      ...
      <Table
        ...
        columnRenderingSettings={{
          ...
          'StartingBalance': {computeLink},
          'EndingBalance': {computeLink},
        }}
        onFetch={load}
        onFetchMore={fetch}
      />
