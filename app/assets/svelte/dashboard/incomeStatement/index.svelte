...
async function fetch(params: Params, offset: number): Promise<FetchResult> {
  return post('/load-income-statement', {
...
async function exportCsv(): Promise<void> {
  ...
  try {
    const json = await post(`/export-income-statement`, {
      params: {...params, currency: CURRENT_CURRENCY},
    })
...
function computeLink(
  ...
  const opts = {period: dataRow[dataColumnIndexById.AccountingPeriod], accounts: [dataRow[dataColumnIndexById.Account]]}
  switch (params.groupBy) {
    case 'Summary':
      return `/income-statement?${generateQueryString({...opts, groupBy: 'Customer'})}`
    case 'Product':
      return `/income-statement?${generateQueryString({...opts, groupBy: 'LineItem', productId: dataRow[dataColumnIndexById.ProductId]})}`
    case 'Customer':
