...
async function fetch(params: Params, offset: number): Promise<FetchResult> {
  return post('/load-direct-cash-flow', {
...
async function exportCsv(): Promise<void> {
  const json = await post('/export-direct-cash-flow', {params})
...
function computeLink(
  ...
  switch (params.groupBy) {
    case 'Summary':
      return `/direct-cash-flow?${generateQueryString({...opts, groupBy: 'Customer'})}`
    case 'Customer':
      return `/direct-cash-flow?${generateQueryString({...opts, groupBy: 'Transaction', customerId: dataRow[dataColumnIndexById.CustomerId]})}`
    case 'Transaction':
      return null
    default:
      return null
