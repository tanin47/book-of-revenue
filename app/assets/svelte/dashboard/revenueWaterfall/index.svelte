...
let totalNumberOfRows: number = 0
let params: Params = {
...
function generateQueryString(
  ...
  const h = overrides?.highlightColumnId ?? highlightColumnId
  if (h) { q.append('h', h)}
...
async function fetch(params: Params, offset: number): Promise<FetchResult> {
  return post('/load-revenue-waterfall', {
...
async function exportCsv(): Promise<void> {
  ...
  try {
    const json = await post(`/export-revenue-waterfall`, {
      params: {...params, currency: CURRENT_CURRENCY},
    })
...
function computeLink(
  ...
  const opts = {
    period: dataRow[dataColumnIndexById.BookedAccountingPeriod],
    highlightColumnId: columnId,
    sorts: [{columnId, direction: 'Desc'}] as Sort[]
  }
  switch (params.groupBy) {
    case 'Summary':
      return `/revenue-waterfall?${generateQueryString({...opts, groupBy: 'Customer'})}`
    case 'Product':
      return `/revenue-waterfall?${generateQueryString({...opts, groupBy: 'LineItem', productId: dataRow[dataColumnIndexById.ProductId]})}`
    case 'Customer':
  ...
  <div class="flex flex-col w-full h-full grow overflow-hidden">
    <div class="flex flex-col grow w-full items-stretch overflow-hidden relative">
      ...
      <Table
        ...
        stickFirstNColumns={params.columns.length}
        columnRenderingSettings={{
          ...DEFAULT_COLUMN_RENDERING_SETTINGS,
          'Total': {computeLink},
          '\\d{4}-\\d{2}': {isRegex: true, computeLink},
        }}
        onFetch={load}
        onFetchMore={fetch}
      />
