...
$: {
  // Only update `visible` when `startIndex` or `endIndex` changes due to scrolling.
  visible = getItems().slice(startIndex, endIndex + 1).map((item, i) => {
...
async function refresh(_items: Item[], _viewportHeight: number) {
  startIndex = -1; // trigger the change in `visible`
  startIndex = 0;
