...
class StripeAccountSelectionRequiredException extends Exception
...
class ErrorHandler @Inject() (
  ...
  override def onServerError(
    ...
    exception match {
      ...
      case e: StripeAccountSelectionRequiredException => logMundaneException(e)
      case e: ValidationException             =>
        logMundaneException(e)
        val _ = trackedExceptionService.create(e)
    ...
    exception match {
      ...
      case _: AuthenticationRequiredException =>
        if (isAjax) {
          ...
          Future(Redirect(routes.AuthController.login(Some(request.uri))))
        }
      case _: StripeAccountSelectionRequiredException =>
        if (isAjax) {
          Future(
            Results.BadRequest(
              Json.obj(
                "errors" -> Seq(
                  messagesApi.apply("validation.requireStripeAccountSelection")(Lang("en"))
                )
              )
            )
          )
        } else {
          Future(Redirect(routes.AuthController.selectStripeAccount()))
        }
