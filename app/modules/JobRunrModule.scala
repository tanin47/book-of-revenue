...
@Singleton
class JobRunrMainStarter @Inject() (
  app: Application,
  lifecycle: ApplicationLifecycle,
  baseConfiguration: JobRunrBaseConfiguration
)(implicit
  ...
  lazy val main: JobRunrMain = {
    if (app.mode == Mode.Prod) {
      val m = new JobRunrMain(app)

      lifecycle.addStopHook(() => Future { m.stop() })
...
class JobRunrModule extends AbstractModule {
  override def configure(): Unit = {
    ...
    bind(classOf[JobRequestScheduler])
      ...
      .asEagerSingleton()
    bind(classOf[JobRunrMain])
      .toProvider(classOf[JobRunrMainStarter])
      .asEagerSingleton()
  }
}
