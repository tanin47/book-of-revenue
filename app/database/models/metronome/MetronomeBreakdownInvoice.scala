...
case class MetronomeBreakdownInvoice(
  id: String,
...
case class RichMetronomeBreakdownInvoice(
  base: MetronomeBreakdownInvoice,
  lineItems: Seq[MetronomeBreakdownLineItem],
)

class MetronomeBreakdownInvoiceTable(tag: Tag) extends Table[MetronomeBreakdownInvoice](tag, Some("metronome"), "breakdowns_invoices") {
  ...
  def * : ProvenShape[MetronomeBreakdownInvoice] = (
    id ::
    environmentType ::
    snapshotTimestamp ::
    invoiceId ::
    ...
    updatedAt ::
      HNil
  ).mapTo[MetronomeBreakdownInvoice]
}
