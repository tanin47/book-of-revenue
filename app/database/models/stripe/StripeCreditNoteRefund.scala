...
case class StripeCreditNoteRefund(
...
case class RichStripeCreditNoteRefund(
  base: StripeCreditNoteRefund,
  refund: Option[RichStripeRefund]
) extends Jsonable {
...
class StripeCreditNoteRefundTable(tag: Tag) extends Table[StripeCreditNoteRefund](tag, Some("stripe"), "credit_note_refund") {
  ...
  def * : ProvenShape[StripeCreditNoteRefund] = (
    stripeAccountId,
    liveMode,
    creditNoteId,
    rank,
