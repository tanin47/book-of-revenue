...
case class RichStripeCreditNote(
  base: StripeCreditNote,
  customerBalanceTransaction: Option[StripeCustomerBalanceTransaction],
  lines: Seq[RichStripeCreditNoteLineItem],
  refunds: Seq[RichStripeCreditNoteRefund],
) extends Jsonable {
...
class StripeCreditNoteTable(tag: Tag) extends Table[StripeCreditNote](tag, Some("stripe"), "credit_note") {
  ...
  def * : ProvenShape[StripeCreditNote] = (
    stripeAccountId,
    liveMode,
    id,
    `type`,
