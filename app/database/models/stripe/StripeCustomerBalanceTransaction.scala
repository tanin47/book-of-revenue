...
import framework.TransactionDetail.BillingActivity.{CreditCustomerBalance, DebitCustomerBalance}
import framework.{Instant, Jsonable}
import play.api.libs.json.{JsObject, Json}
import slick.lifted.{ProvenShape, Rep}

case class StripeCustomerBalanceTransaction(
...
class StripeCustomerBalanceTransactionTable(tag: Tag) extends Table[StripeCustomerBalanceTransaction](tag, Some("stripe"), "customer_balance_transaction") {
  ...
  def * : ProvenShape[StripeCustomerBalanceTransaction] = (
    stripeAccountId,
    liveMode,
    id,
    amount,
