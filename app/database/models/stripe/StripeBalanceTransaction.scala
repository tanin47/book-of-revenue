...
case class StripeBalanceTransaction(
...
case class RichStripeBalanceTransaction(
  base: StripeBalanceTransaction,
  charge: Option[StripeCharge],
)

class StripeBalanceTransactionTable(tag: Tag) extends Table[StripeBalanceTransaction](tag, Some("stripe"), "balance_transaction") {
  ...
  def * : ProvenShape[StripeBalanceTransaction] = (
    stripeAccountId,
    liveMode,
    id,
    amount,
